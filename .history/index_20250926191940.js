...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function onClickUser(username, isGroup = false) {
    ...
    if(isGroup){
      await refreshGroupMessages(group.id); // targetUser = groupe
      startGroupMessagesPolling(group.id); // on créera aussi un polling spécifique
    } else {
  ...
  function startGroupMessagesPolling(group.id){
    stopGroupMessagesPolling();
    groupMessagesInterval = setInterval(() => refreshGroupMessages(group.id), 2000);
  }
